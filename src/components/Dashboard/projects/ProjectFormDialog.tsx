...
export default function ProjectFormDialog({
  ...
  useEffect(() => {
    if (isOpen && !prevIsOpen.current) {
      const newName = mode === "edit" && project ? project.name : "";
      const newDesc =
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      newErrors.name = t("dashboard.projects.form.name") + " is required";
    }
              ...
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-semibold">
                  {mode === "create"
                    ...
                    : t("dashboard.projects.edit.title")}
                </h2>
                <button
                  onClick={onClose}
                  disabled={isLoading}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-card-foreground mb-1.5">
                    {t("dashboard.projects.form.name")}
                  </label>
                  <input
                    type="text"
                    value={name}
                ...
                <div>
                  <label className="block text-sm font-medium text-card-foreground mb-1.5">
                    {t("dashboard.projects.form.description")}
                  </label>
                  <textarea
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder={t(
                      "dashboard.projects.form.descriptionPlaceholder",
                    )}
                    rows={3}
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  >
                    {t("dashboard.projects.delete.cancel")}
                  </button>
                  <button
                    type="submit"
                    disabled={isLoading}
