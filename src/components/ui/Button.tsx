import { staggerItem } from "../../animations";
import { motion } from "framer-motion";
import { Link, type To } from "react-router-dom";

interface ButtonProps {
  text: string;
  onClick?: () => void;
  to?: To;
  type: "button" | "link";
  className?: string;
}
export default function Button({
  text,
  to,
  onClick,
  type,
  className = "",
}: ButtonProps) {
  return (
    <motion.div variants={staggerItem} className={className}>
      {type === "link" && to ? (
        <motion.div
          onClick={onClick}
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
        >
          <Link
            to={to}
            className={`bg-primary text-primary-foreground px-6 py-2 rounded-xl text-base font-semibold inline-flex items-center justify-center gap-2 shadow-lg shadow-primary/25 ${className} cursor-pointer`}
          >
            {text}
          </Link>
        </motion.div>
      ) : (
        <motion.button
          onClick={onClick}
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          className={`bg-primary text-primary-foreground px-8 h-12 rounded-xl text-base font-semibold inline-flex items-center justify-center gap-2 shadow-lg shadow-primary/25 ${className}`}
        >
          {text}
        </motion.button>
      )}
    </motion.div>
  );
}
